...
const getCoopId = async (req, p = getPrisma()) => {
    ...
    if (!first)
        throw new Error("No cooperative found in the system.");
    return first.id;
};
...
const getBaseUrl = (req) => {
    ...
    if (!host)
        return 'https://coop1-03.vercel.app'; // Hard fallback for this project
...
app.get('/auth/callback', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const score = (name) => {
                let s = 0;
                if (name.includes('flash-lite') || name.includes('flash_lite'))
                    s += 30;
...
app.post('/api/documents', async (req, res) => {
    ...
    const document = await getPrisma().document.create({
        data: {
            ...
            author: author || (req.session?.user?.name || 'System'),
...
app.post('/api/events/:id/attend', async (req, res) => {
    ...
    try {
        ...
        if (!tenant)
            return res.status(404).json({ error: 'Tenant record not found for this user' });
        const event = await getPrisma().coopEvent.update({
            where: { id: req.params.id },
            data: {
                attendees: {
...
app.post('/api/ai/triage', async (req, res) => {
    try {
        const genAI = getAI();
        const resolvedModel = req.session?.user?.geminiModel || 'gemini-2.5-flash-lite';
...
app.get('/api/seed', async (req, res) => {
    try {
        ...
        for (const d of documentData) {
            ...
            await p.$executeRawUnsafe(`INSERT INTO "Document" (id, title, category, committee, url, "fileType", author, date, tags, content, "cooperativeId", "createdAt", "updatedAt")
         VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7::timestamptz, ARRAY[]::TEXT[], NULL, $8, now(), now())`, sanitizeUtf8(d.title || "").trim(), sanitizeUtf8(d.category || "").trim(), "", sanitizeUtf8(d.url || "").trim(), sanitizeUtf8(d.fileType || "pdf").trim(), sanitizeUtf8(d.author || "").trim(), new Date(d.date).toISOString(), coopId);
        }
