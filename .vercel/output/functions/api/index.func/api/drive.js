...
async function isFolderWithinRoot(folderId, cache) {
    if (ROOT_FOLDER_IDS.length === 0)
        return false;
...
router.get('/root', async (_req, res) => {
    try {
        ...
        for (const rootId of ROOT_FOLDER_IDS) {
            ...
            allFolders.push(...currentRootFiles.filter((f) => f.mimeType === 'application/vnd.google-apps.folder'));
            allFiles.push(...currentRootFiles.filter((f) => f.mimeType !== 'application/vnd.google-apps.folder'));
        }
        res.json({
...
router.get('/search', async (req, res) => {
    try {
        ...
        const filterPromises = allFiles.map(async (file) => {
            ...
            if (ROOT_FOLDER_IDS.includes(file.id))
                return file;
        ...
        const filteredFiles = results.filter((f) => f !== null);
