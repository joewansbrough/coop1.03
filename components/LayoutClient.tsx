          ...
          {navItems.map((item, index) => {
            ...
            return (
              <React.Fragment key={item.path}>
                ...
                >
                  <i className={`fa-solid ${item.icon} w-5 group-hover:scale-110 transition-transform ${pathname === item.path ? 'text-white' : (item.isAdmin ? 'text-amber-500/70' : 'text-slate-500')}`}></i>
                  ...
                  <>
                    ...
                    <span className="text-slate-600 dark:text-slate-300">
                      {navItems.find(n => n.path === pathname)?.label || 
                       (pathname.includes('/calendar/') ? 'Event Detail' : 
                        pathname.includes('/announcements/') ? 'Announcement' : 'Portal')}
                    </span>
                  </>
                )}
              </div>
            ...
            <div className="relative" ref={profileRef}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-white/5 py-2 animate-in fade-in zoom-in-95 duration-100 overflow-hidden">
                  ...
                  <button onClick={() => { router.push('/documents'); setIsProfileOpen(false); }} className="w-full text-left px-4 py-3 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 flex items-center gap-3 transition-colors">
              ...
              <div className="p-8 text-center">
                {searchQuery ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => { router.push('/documents'); setIsSearchOpen(false); }} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 text-left hover:border-emerald-500 transition-all">
                    ...
                    <button onClick={() => { router.push('/calendar'); setIsSearchOpen(false); }} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 text-left hover:border-emerald-500 transition-all">
