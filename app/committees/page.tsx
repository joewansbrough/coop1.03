...
import { useUser, useCommittees, useTenants, useDocuments } from '../../hooks/useCoopData';
import { MOCK_COMMITTEES } from '../../constants';

export default function CommitteesPage() {
  const { data: user } = useUser();
  const searchParams = useSearchParams();
  const { data: committees = MOCK_COMMITTEES, refetch: fetchCommittees } = useCommittees();
  ...
  const handleAssignMember = async (committeeId: string) => {
    if (isGuest) return;
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500 pb-12 transition-colors duration-200">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl p-8 animate-in zoom-in-95">
            ...
            <form onSubmit={handleAddCommittee} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newCommitteeName} onChange={e => setNewCommitteeName(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Description</label>
                <textarea required value={newCommitteeDesc} onChange={e => setNewCommitteeDesc(e.target.value)} rows={3} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-white resize-none" />
              </div>
              <div>
      ...
      {!selectedId ? (
        ...
        <div className="space-y-6">
          <button 
            ...
            className="text-xs font-black text-slate-400 hover:text-emerald-600 flex items-center gap-2 uppercase tracking-widest transition-all group mb-6"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white dark:bg-slate-900 p-10 rounded-3xl border border-slate-200 dark:border-white/5 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-full text-[10px] font-black uppercase tracking-widest">
              ...
              {showUpload && isAdmin && !isGuest && (
                <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-500/20 p-8 rounded-3xl animate-in slide-in-from-top-4 duration-300">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="font-black text-emerald-900 dark:text-emerald-400 uppercase text-xs tracking-widest">Secretary Submission Portal</h3>
                    <button onClick={() => setShowUpload(false)} className="text-emerald-600"><i className="fa-solid fa-xmark"></i></button>
                  </div>
                  <form onSubmit={handleUploadMinute} className="space-y-4">
                    <div className="bg-white dark:bg-slate-900 border-2 border-dashed border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-8 flex flex-col items-center justify-center text-center group hover:bg-white/50 dark:hover:bg-slate-800/50 transition-colors">
                      <i className="fa-solid fa-file-arrow-up text-3xl text-emerald-300 group-hover:text-emerald-500 mb-3"></i>
                      <p className="text-xs font-bold text-slate-500 dark:text-slate-400">Drop PDF meeting minutes here or <span className="text-emerald-600 underline cursor-pointer">browse</span></p>
                    </div>
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-white/5">
                 ...
                 {!showContact ? (
                   <button 
                     ...
                     disabled={isGuest}
                     className={`w-full py-3 rounded-2xl font-black text-sm transition-all ${isGuest ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-emerald-600 text-white hover:bg-emerald-700 hover:-translate-y-0.5 active:scale-95'}`}
                   >
                   ...
                   <form onSubmit={handleSendMessage} className="space-y-4 animate-in fade-in slide-in-from-top-2">
                     ...
                     <div className="flex gap-2">
                       ...
                       <button 
                        ...
                        className={`flex-1 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${sentStatus ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-900 dark:bg-emerald-600 text-white hover:bg-black dark:hover:bg-emerald-700 active:scale-95'} ${isGuest ? 'opacity-50 cursor-not-allowed' : ''}`}
                       >
              ...
              <div className="bg-slate-900 dark:bg-slate-800 p-8 rounded-3xl text-white relative overflow-hidden group">
                ...
                <button 
                  disabled={isGuest}
                  className={`w-full py-3 rounded-2xl font-black text-sm transition-all ${isGuest ? 'bg-white/5 text-slate-500 cursor-not-allowed' : 'bg-emerald-600 text-white hover:bg-emerald-700 active:scale-95'}`}
                >
