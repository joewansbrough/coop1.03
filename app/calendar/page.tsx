...
export default function CalendarPage() {
  ...
  const { data: events = MOCK_EVENTS, refetch: fetchEvents } = useEvents();
  ...
  const nextEvent = upcomingEventsSorted[0];
  const upcomingEvents = upcomingEventsSorted.slice(0, 5);
                ...
                >
                  ...
                  <div className="mt-1.5 space-y-1">
                    {hasEvents.map(e => (
                      <div 
                        key={e.id} 
                        onClick={(ev) => { ev.stopPropagation(); router.push(`/calendar/${e.id}`); }}
