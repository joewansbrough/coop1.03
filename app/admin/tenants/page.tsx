...
import { useUser, useTenants, useUnits } from '../../../hooks/useCoopData';
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-12 transition-colors duration-200">
      ...
      <div className="sm:hidden space-y-3">
        {unitGroups.map(group => (
          <div key={group.unit.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden transition-colors duration-200">
            {/* Unit header */}
            ...
            {group.members.map((member, idx) => (
              <div
                ...
                onClick={() => router.push(isAdmin ? `/admin/tenants/${member.id}` : `/directory`)}
              >
        ...
        {waitlistMembers.length > 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden transition-colors duration-200">
        ...
        {unitGroups.length === 0 && waitlistMembers.length === 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/5 px-6 py-12 text-center text-slate-400 text-xs italic font-medium transition-colors duration-200">
      ...
      {/* ── Desktop: Table ── */}
      <div className="hidden sm:block bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm transition-colors duration-200">
