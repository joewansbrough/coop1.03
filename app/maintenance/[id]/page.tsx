...
import { useUser, useMaintenance, useUnits, useTenants } from '../../../hooks/useCoopData';

export default function MaintenanceDetailPage() {
  const params = useParams();
  const requestId = params.id as string;
  const { data: user } = useUser();
  const { data: requests = [], refetch: fetchRequests } = useMaintenance();
  ...
  const userUnitId = user?.tenantId ? units.find(u => u.currentTenantId === user.tenantId)?.id : null;
  ...
  if (request && !isAdmin && request.unitId !== userUnitId) {
    return (
      <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-white/5">
        ...
        <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-2 uppercase tracking-tight">Access Restricted</h2>
        ...
        <Link href="/maintenance" className="mt-8 inline-block bg-slate-900 dark:bg-emerald-600 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">Return to Queue</Link>
      </div>
    );
  }
  ...
  const addNote = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newNote.trim() || isLocked || isGuest) return;

    setIsSavingNote(true);

    const note: MaintenanceNote = {
      id: `n${Date.now()}`,
      author: isAdmin ? 'Board Admin' : user.name || 'Member Note',
  ...
  const addExpense = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newItem.trim() || !newCost || isLocked || isGuest) return;
  ...
  const handleReopen = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reopenReason.trim() || isGuest) return;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 animate-in fade-in duration-500 transition-colors duration-200">
      <div className="flex items-center justify-between gap-4 text-slate-500 text-sm mb-2">
        <Link href="/maintenance" className="hover:text-emerald-600 transition-colors flex items-center gap-1 font-bold uppercase tracking-widest text-[10px]">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/5 space-y-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 w-full gap-3">
              {(Object.values(RequestStatus)).map((status) => {
                ...
                return (
                  <button
                    key={status}
                    disabled={((isLocked && status !== RequestStatus.IN_PROGRESS) || !isAdmin || isGuest) && !isActive}
                    onClick={() => isAdmin && !isGuest && handleStatusChange(status)}
                    className={`py-3 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                      isActive 
                        ? `${
                            ...
                            'bg-blue-600'
                          } text-white scale-105 z-10 shadow-lg shadow-black/10` 
                        : isAdmin && !isGuest
                          ? 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-600 opacity-60 hover:opacity-100 border border-slate-100 dark:border-white/5'
                          : 'bg-slate-50 dark:bg-slate-800 text-slate-300 dark:text-slate-700 opacity-30 cursor-not-allowed border border-slate-100 dark:border-white/5'
                    }`}
                  >
                    {status}
                  </button>
                ...
                {(Object.values(MaintenancePriority)).map((p) => {
                  ...
                  return (
                    <button 
                      key={p}
                      disabled={isGuest}
                      onClick={() => !isGuest && persistUpdate({ ...request, priority: p, updatedAt: new Date().toISOString() })}
                      className={`py-3 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                        isActive 
                          ? `${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-white/5 shadow-sm">
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {isAdmin && !isLocked && !isGuest && <button onClick={() => setIsEditingCategories(!isEditingCategories)} className="text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border border-dashed border-slate-300 dark:border-slate-700 text-slate-400 hover:border-emerald-500 transition-colors flex items-center gap-2"><i className="fa-solid fa-plus"></i> Edit Service Type</button>}
            </div>

            {isEditingCategories && (
              <div className="flex flex-wrap gap-2 mb-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-white/10 animate-in slide-in-from-top-2 duration-200">
            ...
            <h1 className="text-4xl font-black text-slate-900 dark:text-white leading-tight mb-8 uppercase tracking-tight">{request.description}</h1>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-8 border-t border-slate-50 dark:border-white/5">
              <Link href={`/admin/units/${unit?.id}`} className="flex items-center gap-4 group p-5 bg-slate-50 dark:bg-slate-950/30 rounded-2xl border border-transparent hover:border-emerald-500/50 transition-all">
              ...
              {tenant ? (
                <Link href={`/admin/tenants/${tenant.id}`} className="flex items-center gap-4 group p-5 bg-slate-50 dark:bg-slate-950/30 rounded-2xl border border-transparent hover:border-blue-500/50 transition-all">
          ...
          <section className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-white/5 shadow-sm overflow-hidden transition-colors">
            ...
            <div className="p-6">
              ...
              {!isLocked && (
                <form onSubmit={addNote} className="pt-4">
                  <textarea
                    className="w-full bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-white/10 rounded-2xl p-4 text-sm focus:ring-2 focus:ring-emerald-500 outline-none text-slate-800 dark:text-slate-200 font-medium resize-none h-24"
                    placeholder={isAdmin ? "Log a new update or member contact..." : "Add a comment for the maintenance committee..."}
                    value={newNote}
                    onChange={(e) => setNewNote(e.target.value)}
                  />
                  <div className="flex justify-end mt-3">
                    <button type="submit" disabled={isSavingNote || !newNote.trim() || isGuest} className="bg-slate-900 dark:bg-emerald-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50">
                      {isSavingNote ? 'Syncing...' : 'Post Update'}
                    </button>
                  </div>
                </form>
              )}
          ...
          {isAdmin && (
            <section className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden transition-colors">
              ...
              <div className="p-6">
                {!isLocked && !isGuest && (
                  <form onSubmit={addExpense} className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 p-4 bg-slate-50 dark:bg-slate-950/30 rounded-2xl border border-slate-100 dark:border-white/5">
                     <input type="text" placeholder="Service/Part" className="col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-white" value={newItem} onChange={e => setNewItem(e.target.value)} />
                     <input type="number" placeholder="0.00" className="col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-emerald-500 text-slate-900 dark:text-white" value={newCost} onChange={e => setNewCost(e.target.value)} />
                     <button type="submit" className="bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 px-4 hover:bg-emerald-700 active:scale-95 transition-all"><i className="fa-solid fa-plus"></i> Add Item</button>
                  </form>
                )}
                   ...
                   <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                         ...
                         {(!request.expenses || request.expenses.length === 0) && (
                           <tr><td colSpan={3} className="px-4 py-8 text-center text-[10px] font-black uppercase text-slate-300 italic">No costs recorded.</td></tr>
                         )}
                      </tbody>
                   </table>
                </div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 dark:bg-slate-950 text-white p-8 rounded-3xl shadow-lg border border-white/5">
             ...
             <div className="space-y-3">
                ...
                {isAdmin && !isGuest && (
                  <button onClick={() => alert("Resident has been notified.")} className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center gap-3 hover:bg-white/10 transition-colors">
      ...
      {showReopenModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl p-8 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium">Please provide a reason for reopening this record. This will be logged in the activity feed.</p>
            <form onSubmit={handleReopen} className="space-y-4">
              <textarea 
                required 
                value={reopenReason} 
                onChange={e => setReopenReason(e.target.value)}
                className="w-full h-32 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl p-4 text-sm outline-none focus:ring-2 focus:ring-amber-500 text-slate-900 dark:text-white"
                placeholder="Reason for reopening..."
              />
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowReopenModal(false)} className="flex-1 py-3 text-xs font-black uppercase text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">Cancel</button>
                <button type="submit" className="flex-1 py-3 bg-amber-500 text-white rounded-xl text-xs font-black uppercase hover:bg-amber-600 transition-all active:scale-95">Confirm Reopen</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showStatusConfirm && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-8 animate-in zoom-in-95 duration-200 text-center shadow-2xl">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 font-medium">
              Are you sure you want to move this request to <span className="font-black text-slate-700 dark:text-slate-300 uppercase">{pendingStatus}</span>?
            </p>
            <div className="flex gap-3">
              <button 
