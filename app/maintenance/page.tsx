...
import { useUser, useMaintenance, useUnits } from '../../hooks/useCoopData';
...
export default function MaintenancePage() {
  ...
  const userUnitId = user.tenantId ? units.find(u => u.currentTenantId === user.tenantId)?.id : 'u1';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      unitId: isAdmin ? unitId : userUnitId,
      category: category,
      description,
  ...
  const confirmRequestStatus = async () => {
    if (!pendingRequest || isGuest) return;
              ...
              <div>
                ...
                {isAdmin ? (
                  ...
                  <div className="w-full bg-slate-50 dark:bg-slate-800 border dark:border-white/5 rounded-xl p-3 text-sm font-bold text-slate-800 dark:text-slate-200">
                    Unit {units.find(u => u.id === userUnitId)?.number || '—'} (Assigned)
                  </div>
                )}
              </div>
                ...
                <table className="w-full text-left min-w-[700px]">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {openRequests.length > 0 ? openRequests.map(req => (
                      <tr key={req.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => router.push(isAdmin ? `/admin/maintenance/${req.id}` : `/maintenance/${req.id}`)}>
