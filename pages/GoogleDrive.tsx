...
const GoogleDrive: React.FC = () => {
  ...
  const handleOpenPicker = () => {
    if (!config || !window.google || !window.gapi) {
  ...
  const pickerCallback = (data: any) => {
    if (data.action === window.google.picker.Action.PICKED) {
      ...
      setIsConnected(true);
      // Add picked file to our local list
      setFiles(prev => [
        { 
          id: doc.id, 
          name: doc.name, 
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-sm">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-3 shadow-lg shadow-blue-500/20 active:scale-95"
        >
          <img src="https://www.google.com/favicon.ico" className="w-4 h-4 brightness-0 invert" alt="" />
          {files.length > 0 ? 'Select More Files' : 'Browse Google Drive'}
        </button>
      </header>
